"""
LogViewer - Standalone debug log viewer for bot framework
...
class LogViewer:
    ...
    def __init__(self, root, initial_device=None, initial_session=None):
        ...
        """
        self.root = root
        self.root.title("Bot Framework - Log Viewer")
        ...
        # Session data
        self.sessions_data: list[dict] = []  # List of session info dicts
    ...
    def load_sessions(self):
        ...
        if sessions:
            ...
            # Auto-select session if this is the initial device selection
            if self.initial_device and self.selected_device == self.initial_device:
                if self.initial_session:
                    ...
                    if self.initial_session in session_ids:
                        ...
                        # Session not found, select the most recent (first in list, since ordered DESC)
                        idx = 0
                else:
                    # No session specified, select the most recent (first in list)
                    idx = 0
                ...
                # Clear initial values so they don't get reapplied
    ...
    def add_inline_image(self, parent_frame, screenshot):
        ...
        try:
            ...
            img_label.image = photo  # type: ignore[attr-defined]  # Keep reference to prevent GC
            img_label.pack()
