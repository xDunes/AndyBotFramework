...
# Add parent directory to path so we can import state_manager
...
def get_all_bots():
    ...
    try:
        ...
        for bot in all_bots:
            ...
            # Convert is_running from SQLite integer (0/1) to Python boolean
...
def get_device_screenshot(device_name):
    """Get screenshot for a specific device (as base64 PNG)"""
    try:
        ...
        # Use PIL to encode PNG properly with RGB color order
        ...
        return jsonify({
            'success': True,
            'screenshot': f'data:image/png;base64,{base64_data}',
...
@app.route('/api/command/shortcut', methods=['POST'])
def send_shortcut_command():
    """Send shortcut command to device(s)"""
    try:
        data = request.json
        device_name = data.get('device_name')
        apply_mode = data.get('apply_mode', 'current')
        ...
        if shortcut == 'start_stop':
            ...
            # Handle other shortcuts (min_fans, max_fans)
            shortcut_map = {
                'min_fans': {'type': 'assist_shortcut', 'name': 'min_fans'},
                'max_fans': {'type': 'assist_shortcut', 'name': 'max_fans'}
            }
            ...
            command_data = shortcut_map[shortcut]
                ...
                if last_screenshots.get(device_name) != screenshot_timestamp:
                    ...
                    if screenshot is not None:
                        try:
                            ...
                            # Encode as JPEG for faster transfer
                            img = Image.fromarray(screenshot)
                            buffer = io.BytesIO()
